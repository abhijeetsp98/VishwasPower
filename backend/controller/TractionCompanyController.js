...
export const setapproveCompanyStage = async (req, res) => {
  try {
    ...
    // ── Guard 1: Fetch the current project state before approving ──
...
export const editCompanyName = async (req, res) => {
  try {
    ...
    const existingCompany = await TractionCompany.findOne({ companyName: oldCompanyName });
    if (!existingCompany) {
      return res.status(404).json({ 
    ...
    const duplicateCompany = await TractionCompany.findOne({ companyName: newCompanyName });
    if (duplicateCompany) {
      return res.status(400).json({ 
    ...
    // Also update the company name in Traction collection if any projects exist
    try {
      await Traction.updateMany(
